...
recommend using the default of 2 for top variable features. 
#### Sample Correlations
This heat map shows how similar each sample is to each other sample in your
data (samples are ploted on both the x and y axis, so are identical to themself). 
The samples are clustered together based on similarity. Ideally, you would
expect your samples to cluster and be more similar to samples from the same
condition rather than from the sample batch. If there is clear visual clustering
of the batch in your raw data, you should apply a batch correction method. Your
batch corrected assay should have stronger clustering by condition.

#### Heatmap
The heatmap shoes the sample clustering on the x axis and the y axis is each
gene. The values on the heatmap represent gene expression. The dendogram on the
...
### Dendograms
#### Dendogram
This tab shows a more detailed dendogram as seen in the heatmap. When samples
...
Currently, you may only select one covairate to display at a time.

#### Circular Dendogram
This circularizes the previous dendogram to better show relatedness of all 
branches on the dendogram without the appearance of great distance of the 
samples at the top and the bottom of the chart.

### PCA Analysis
You may select multiple assays to plot PCS analysis side by side. Therefore, 
select your raw count assay and batch corrected assay, the batch variable as shape, condition as color, and 2 top variable features. Review the clustering pattern. 
In data sets where a strong batch effect is seen, similar shapes will all cluster together. Whereas if batch effect is not seen, shapes should be dispered
throughout and hopefully you see clustering by condition (color).
