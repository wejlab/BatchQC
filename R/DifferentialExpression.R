...
#'
#' @export
analyze_SE = function(se,method,conditons,assay_to_analyze,output_assay_name) {
  se=se
  if (method=='wilcox') {
    output_assay_name=findMarkers(se, se@colData@data$conditons,
                                 test.type=method, pval.type="all", lfc=1, subset.row=chosen)
    
  }
  else if (method=='DESeq') {
    se@assays@data[[output_assay_name]]=GetNormalizedMat(se@assays@data[[assay_to_analyze]],
